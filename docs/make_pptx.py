...
tags = ["Vue 3", "Spring Boot 3.2", "PostgreSQL 16",
        "Anthropic Claude API", "Java 21 LTS"]
total_w = sum([1.3,1.9,1.9,2.5,1.6]) + 4*0.15
start_x = (13.33 - total_w) / 2
widths  = [1.3, 1.9, 1.9, 2.5, 1.6]
cx = start_x
for tag, w in zip(tags, widths):
    badge(s, tag, cx, 4.45, w, 0.42, bg=BLUE, size=12)
    cx += w + 0.15

tx(s, "2026년 7월  |  개발팀  |  Ver 2.0",
...
layers = [
    (ACCENT, "Frontend", "Vue 3 SPA\n(Nginx 정적 서빙)",
     ["KPI Cards × 4", "Chart.js 차트 × 4", "Lot / 주문 테이블", "AI 채팅 박스"]),
    (BLUE,   "Backend",  "Spring Boot 3.2\n:8080",
     ["REST API 8개", "AiChatService", "SQL 가드레일 6종", "HikariCP 커넥션 풀"]),
    (NAVY,   "Data",     "PostgreSQL 16\n:5432",
     ["5개 테이블", "6개 집계 View", "인덱스 14개", "자동 updated_at 트리거"]),
]
...
s = prs.slides.add_slide(BLANK)
rect(s, 0, 0, 13.33, 7.5, LIGHT)
header(s, "12  향후 로드맵", "Future Roadmap")
...
roadmap = [
    ...
    ("4단계", "2주",  GREEN,  "알림 & 리포팅",
     ["임계값 경보 (Slack/이메일)", "PDF 자동 리포트", "구독형 일간 리포트"]),
]
...
tx(s,
   "산출물:  specification.md  |  specification_enterprise.md  |  postgresql_setup_guide.md  |  presentation.pptx",
